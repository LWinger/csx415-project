...
rename_cols <- function(dataframe_x){
  #fix all datasets since R read in first entry as header
  #add first row as header
  fix = sub("X\\.0","0", colnames(dataframe_x))
  fix = sub("X","", fix)
  dataframe_x <- rbind(as.numeric(fix),dataframe_x)
...
add_test_rul <- function(dataframe_test, ground_truth){
  ...
  #fix ground truth (R read in first entry as header)
  ground_truth <- rbind(strtoi(sub("[A-Z]","", colnames(ground_truth))),ground_truth)
